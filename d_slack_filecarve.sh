...
if [ -f "$OUTDIR/carving/foremost_slack/foremost_slack_audit.txt" ]; then
    echo ""
    egrep '(FILES EXTRACTED|:=)' $OUTDIR/carving/foremost_slack/foremost_slack_audit.txt
else
    echo "File $OUTDIR/carving/foremost_slack/foremost_slack_audit.txt does not exist..."
fi
