...
if [ ! -f "$OUTDIR/d_timeline/d_timeline.csv" ]; then
    if [[ $TIMELINE_START =~ ^[0-9]{4}-[0-9]{2}-[0-9]{2}$ ]] && [[ $TIMELINE_END =~ ^[0-9]{4}-[0-9]{2}-[0-9]{2}$ ]]; then
        ...
        mactime -z UTC -y -d -b $BODYFILE > $OUTDIR/d_timeline/d_timeline.csv
    fi
else
    echo "$OUTDIR/d_timeline/d_timeline.csv already exists - moving on..."
fi
...
if [ -f "$OUTDIR/d_timeline/d_timeline.csv" ] && [ -f $TIMELINE_REDUCE ] && [ ! -f $OUTDIR/d_timeline/d_timeline_filtered.csv ]; then
    ...
    echo "File $OUTDIR/d_timeline/d_timeline.csv, or $OUTDIR/d_timeline/d_timeline_filtered.csv, or $TIMELINE_REDUCE does not exist - exiting..."
    exit 1
fi
