...
if [ -f "$OUTDIR/carving/volatility/rootkits/ssdt.txt" ]; then
    ...
    echo ""
    vol.py --profile=$PROFILE -f $MEMPATH ssdt > $OUTDIR/carving/volatility/rootkits/ssdt_raw.txt
    egrep -v '(ntoskrnl|win32k)' $OUTDIR/carving/volatility/rootkits/ssdt_raw.txt > $OUTDIR/carving/volatility/rootkits/ssdt_filtered.txt
fi
