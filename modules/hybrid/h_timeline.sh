...
if [ ! -f "$OUTDIR/h_timeline/h_timeline.csv" ]; then
    if [[ $TIMELINE_START =~ ^[0-9]{4}-[0-9]{2}-[0-9]{2}$ ]] && [[ $TIMELINE_END =~ ^[0-9]{4}-[0-9]{2}-[0-9]{2}$ ]]; then
        echo "Date $TIMELINE_START and $TIMELINE_END are both in the correct format (YYYY-MM-DD) - creating mactime timeline using this range..."
        mactime -z UTC -y -d -b $BODYFILE $TIMELINE_START..$TIMELINE_END > $OUTDIR/h_timeline/h_timeline.csv
    else
        echo "Date $TIMELINE_START or $TIMELINE_END is invalid format (YYYY-MM-DD) - creating mactime timeline without range..."
        mactime -z UTC -y -d -b $BODYFILE > $OUTDIR/h_timeline/h_timeline.csv
    fi
else
    echo "$OUTDIR/h_timeline/h_timeline.csv already exists - moving on..."
fi
...
if [ -f "$OUTDIR/h_timeline/h_timeline.csv" ] && [ -f $TIMELINE_REDUCE ] && [ ! -f $OUTDIR/h_timeline/h_timeline_filtered.csv ]; then
    ...
    echo "File $OUTDIR/h_timeline/h_timeline.csv, or $OUTDIR/h_timeline/h_timeline_filtered.csv, or $TIMELINE_REDUCE does not exist - exiting..."
    exit 1
fi
